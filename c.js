...
window.addEventListener("unhandledrejection",(_ev)=>{
    const _m=String(_ev?.reason?.message||_ev?.reason||"");
    if(/Extension context invalidated|EXTENSION_CONTEXT_INVALIDATED/i.test(_m)){
        __abContextInvalid=true;
        try{_ev.preventDefault();}catch(_){}
    }
});
